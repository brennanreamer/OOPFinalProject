...
void Trainer::wild_battle(Trainer user, Pokemon& opp) {
	...
	int poke_choice = 0;
	int user_feint_count = 0;
	while (user_feint_count != user.num_pokemon()) {
		while ((user.pokemon[poke_choice].show_health() > 0) && (opp.show_health() > 0)) {
			//Display info on pokemon
			cout << "-------------------" << endl;
			...
			cin >> move_choice;
			move_choice -= 1;
			if ((move_choice != user.pokemon[poke_choice].num_moves()) && (user.pokemon[poke_choice].moves[move_choice].show_pp() > user.pokemon[0].show_PP())) { //if chosen move requires more PP than pokemon has
				cout << "Not enough PP, please choose a different move" << endl;
				for (int i = 0; i < user.pokemon[poke_choice].num_moves(); i++) {
					cout << i + 1 << " - " << user.pokemon[poke_choice].moves[i].get_attack() << endl;
				}
			...
			if (move_choice == user.pokemon[0].num_moves())
			{
				if (pokeballs > 0) {
					...
					if (catch_chance == 0)
					{
						...
						catch_pokemon(opp);
						throw_pokeball();
						user_feint_count = user.num_pokemon();
						break;
					}
			...
			if (move_choice < user.pokemon[0].num_moves()) {
				...
				opp.set_health(user.pokemon[poke_choice].moves[move_choice].get_damage());
				user.pokemon[0].set_PP(user.pokemon[poke_choice].moves[move_choice].show_pp());
				//Opp Attack
				cout << "User took " << opp.moves[opp_choice].get_damage() << " damage" << endl;
				user.pokemon[poke_choice].set_health(opp.moves[opp_choice].get_damage());
			...
			if (user.pokemon[poke_choice].show_health() <= 0) { //If user's pokemon has fainted
				...
				if (user_feint_count == user.num_pokemon()) {
					cout << "All your pokemon have feinted." << endl;
					cout << "You have lost the battle." << endl;
					break;
				}
		...
		if (opp.show_health() <= 0) {
			int user_feint_count = user.num_pokemon();
			cout << "Wild " << opp.show_name() << " has feinted." << endl;
			//Experience Gain
			int exp_gain = 20 * opp.show_level(); // amount of exp gained for winning battle
			cout << "Your " << pokemon[poke_choice].show_name() << " has gained " << exp_gain << " exp. points!" << endl;
			pokemon[poke_choice].exp_earn(exp_gain);
			break;
		}
